...
total = birthdate[0].to_i +
	...
	new_total = total_text[0].to_i + total_text[1].to_i

	if new_total > 9 
			total_text = new_total.to_s
			new_total = new_total[0].to_i + new_total[1].to_i
	end
	path_number = new_total
end

def birth_message(path_number)
	case path_number
	when 1 
...
puts "Your Birth Path number is #{get_birth_path(birthdate)} #{birth_message(get_birth_path(birthdate))} "